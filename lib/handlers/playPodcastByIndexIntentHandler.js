...
module.exports = {
  ...
  async handle (handlerInput) {
    ...
    try {
      ...
      const speechText = t('SPEECH_INVALID_EPISODE_INDEX', {
        postProcess: 'sprintf',
        sprintf: {
          maxEpisodeCount: podcast.maxEpisodeCount
        }
      })
    ...
    const speechText = t('SPEECH_START_PLAYING_EPISODE_AT', {
      postProcess: 'sprintf',
      sprintf: {
    ...
    const cardText = t('SPEECH_START_PLAYING_EPISODE_AT', {
      postProcess: 'sprintf',
      sprintf: {
    ...
    return podcast.response
      .speak(speechText)
      .withSimpleCard(t('CARD_TITLE_PLAYING_EPISODE_AT', {
        postProcess: 'sprintf',
        sprintf: {
          podcastName: podcast.name,
          episodeIndex: position
